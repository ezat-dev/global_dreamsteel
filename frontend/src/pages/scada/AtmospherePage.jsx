  ...
  return (
    ...
    <div className="at-page hmi-dark">
      {/* 배관 부속을 초록으로 물들이는 색 행렬. CSS의 filter: url(#at-green)이 이걸 부른다.
          ...
          = 0.0824 / 0.5020 / 0.2392). 그 색에 밝기(luminance)로 낸 배수를 곱하는데,
          배수 폭이 0.8 ~ 1.2로 좁다. 그래서 어디를 찍어도 #15803d 계열(#127135 ~ #199A49)
          안에 들어오고, 그 안에서만 명암이 져 모양은 남는다.

          폭을 두 번 줄였다. 처음에는 밝기를 1.3배로 키워 초록 한 축에 실었는데 원래 그림이
          밝은 은회색이라 G가 최대치를 넘겨 형광이 됐고, 다음에는 0.35~1.35로 했더니
          그 색으로 안 보이고 밝은 초록이 섞였다. 이 값을 다시 넓히면 같은 일이 반복된다.

          한 줄의 앞 셋은 (그 채널 값 × 0.4 × 밝기 계수 0.2126/0.7152/0.0722), 맨 뒤는
          (그 채널 값 × 0.8)인 고정분이다. A줄은 그대로 둔다 — 투명한 곳이 채워지면
          그림이 상자가 된다. */}
      <svg className="at-filter-defs" aria-hidden="true">
        <filter id="at-green" colorInterpolationFilters="sRGB">
          <feColorMatrix
            type="matrix"
            values="0.00701 0.02357 0.00238 0 0.06592
                    0.04269 0.14361 0.01450 0 0.40160
                    0.02034 0.06843 0.00691 0 0.19136
                    0       0       0       1 0"
          />
        </filter>
      </svg>
