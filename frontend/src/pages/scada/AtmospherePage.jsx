  ...
  return (
    ...
    <div className="at-page hmi-dark">
      {/* 배관 부속을 초록으로 물들이는 색 행렬. CSS의 filter: url(#at-green)이 이걸 부른다.
          ...
          배수는 0.35 ~ 1.35 사이라 어두운 곳은 짙은 초록, 밝은 곳은 조금 밝은 초록이 되고
          어디서도 넘치지 않는다. 명암이 남아서 초록이 되어도 밸브·모터 모양으로 보인다.

          처음에는 밝기를 1.3배로 키워 초록 한 축에 실었는데, 원래 그림이 밝은 은회색이라
          G가 최대치를 넘겨 형광 초록이 됐다. 그래서 색을 먼저 정하고 밝기로 흔드는
          지금 방식으로 바꿨다.

          한 줄의 앞 셋은 (그 채널 값 × 밝기 계수 0.2126/0.7152/0.0722), 맨 뒤는
          (그 채널 값 × 0.35)인 고정분이다. A줄은 그대로 둔다 — 투명한 곳이 채워지면
          그림이 상자가 된다. */}
      <svg className="at-filter-defs" aria-hidden="true">
        <filter id="at-green" colorInterpolationFilters="sRGB">
          <feColorMatrix
            type="matrix"
            values="0.01752 0.05893 0.00595 0 0.02884
                    0.10673 0.35903 0.03624 0 0.17570
                    0.05085 0.17108 0.01727 0 0.08372
                    0       0       0       1 0"
          />
        </filter>
      </svg>
